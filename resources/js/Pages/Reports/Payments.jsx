...
export default function Payments({ payments, filters, isShadowUser, accounts }) {
    const { auth } = usePage().props;
    const { t, locale } = useTranslation();
    const [showFilters, setShowFilters] = useState(false);
    const [isDownloading, setIsDownloading] = useState(false);

    // Safe access to payments data with fallbacks
    const safePayments = payments?.data || [];
    console.log(safePayments);
    
    const paginationData = payments || {
        data: [],
        links: [],
        from: 0,
        to: 0,
    ...
    const handleFilter = () => {
        ...
        if (filterForm.data.account_id) {
            queryParams.account_id = filterForm.data.account_id;
        }
        router.get(route("reports.transaction"), queryParams, {
            preserveScroll: true,
            preserveState: true,
            replace: true,
        });
    ...
    const clearFilters = () => {
        filterForm.setData({ search: "", start_date: "", end_date: "", account_id: "" });
        setTimeout(() => {
            router.get(route("reports.transaction"), {}, {
                preserveScroll: true,
                preserveState: true,
            });
        }, 0);
    ...
    const hasActiveFilters = () => {
        return filterForm.data.search || filterForm.data.start_date || filterForm.data.end_date || filterForm.data.account_id;
    };
    ...
    const fetchAllPaymentsForExport = async () => {
        try {
            const response = await axios.get(route('reports.transaction.export'), {
                params: {
                    search: filterForm.data.search,
                    start_date: filterForm.data.start_date,
                    end_date: filterForm.data.end_date,
                    account_id: filterForm.data.account_id
                }
            });
            return response.data.payments;
        } catch (error) {
            console.error('Error fetching all payments:', error);
            toast.error('Failed to fetch all payment data');
            throw error;
        }
    };

    const calculateTotals = (paymentsData) => {
    ...
    const prepareExportData = (paymentsData) => {
    ...
    const downloadCSV = async () => {
        try {
            ...
            const exportData = prepareExportData(allPayments);
            ...
            csvRows.push(`Account,${filterForm.data.account_id ? accounts?.find(a => a.id == filterForm.data.account_id)?.name || filterForm.data.account_id : 'All'}`);

            // Add summary section
            csvRows.push(''); // Empty row
            csvRows.push('SUMMARY STATISTICS');
            const totals = calculateTotals(allPayments);
            ...
            toast.success(`${allPayments.length} payments exported successfully to CSV`);
        } catch (error) {
    ...
    const downloadExcel = async () => {
        try {
            ...
            const filterData = [
                ...
                { 'Filter': 'Account', 'Value': filterForm.data.account_id ? accounts?.find(a => a.id == filterForm.data.account_id)?.name || filterForm.data.account_id : 'All' }
            ];
            ...
            toast.success(`${allPayments.length} payments exported successfully to Excel`);
        } catch (error) {
    ...
    const downloadPDF = async () => {
        try {
            ...
            doc.text(`Account: ${filterForm.data.account_id ? accounts?.find(a => a.id == filterForm.data.account_id)?.name || filterForm.data.account_id : 'All'}`, 14, 41);
            ...
            toast.success(`${allPayments.length} payments exported successfully to PDF`);
        } catch (error) {
    ...
    const fallbackExportWithPaginatedData = () => {
        try {
            ...
            const exportData = prepareExportData(safePayments);
            ...
            const totals = calculateTotals(safePayments);
            csvRows.push(`Total Payments (This Page),${totals.totalPayments}`);
            csvRows.push(`Total Amount (Tk),${totals.totalAmount.toFixed(2)}`);
            csvRows.push(`Total Cash,${totals.totalCash.toFixed(2)}`);
            csvRows.push(`Total Mobile Banking,${totals.totalCard.toFixed(2)}`);
            csvRows.push(`Total Bank Transfer,${totals.totalBank.toFixed(2)}`);
            ...
            const url = URL.createObjectURL(blob);
            link.href = url;
            link.download = `payments_report_paginated_${formatDateForFilename()}.csv`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(url);
                    ...
                    <div className="p-4 border-t border-base-content/5">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <fieldset className="fieldset">
                                ...
                                <select
                                    ...
                                    value={filterForm.data.account_id}
                                    onChange={(e) => filterForm.setData("account_id", e.target.value)}
                                >
                        ...
                        {hasActiveFilters() && (
                            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-gray-600">
                                ...
                                {filterForm.data.account_id && (
                                    <span className="badge badge-outline badge-sm">
                                        Account: {accounts?.find(a => a.id == filterForm.data.account_id)?.name || filterForm.data.account_id}
                                    </span>
                                )}
                            </div>
                        )}
