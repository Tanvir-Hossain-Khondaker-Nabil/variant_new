...
import { Frown, Plus, Trash2, Eye, Search, Edit, Check, X, Wallet, AlertCircle, DollarSign, History, Landmark, Smartphone, CreditCard, Download, Filter, ChevronUp, ChevronDown } from "lucide-react";
...
export default function SalesIndex({ sales, filters, isShadowUser, accounts }) {
    ...
    const prepareExportData = () => {
        ...
        return salesData.map(sale => ({
            ...
            'Status': sale.status,
            'Payment Status': sale.payment_status || sale.status,
            'Payment Type': sale.payment_type || 'N/A',
            'Sale Type': sale.type || 'N/A',
    ...
    const downloadCSV = () => {
        try {
            ...
            link.href = url;
            link.download = `sales_report_${formatDateForFilename()}.csv`;
            link.click();
    ...
    const downloadExcel = () => {
        try {
            ...
            XLSX.writeFile(wb, `sales_report_${formatDateForFilename()}.xlsx`);
    ...
    const downloadPDF = () => {
        try {
            ...
            // Save PDF
            doc.save(`sales_report_${formatDateForFilename()}.pdf`);
